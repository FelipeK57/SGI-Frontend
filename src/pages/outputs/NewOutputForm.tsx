...
import type { Client, NewOutput, Part, PartOutput, UnitPart } from "../../Clases";
import { fetchClients } from "../../services/clientService";
import { addToast, Autocomplete, AutocompleteItem, Button, DatePicker, Form, Input, NumberInput } from "@heroui/react";
import { SearchIcon } from "../part/Parts";
import { SelectorIcon } from "../client_quotation/NewClientQuotation";
import { getUnitPartByPartNumber, getUnitPartBySerial } from "../../services/unitsPartService";
...
export const NewOutputForm = () => {
  ...
  const [unitPartWarranty, setUnitPartWarranty] = useState<PartOutput[] | null>(null);
  ...
  const handleSelectionChange = (key: Key | null) => {
    ...
    if (unitPartsAdded.length > 0 || unitPartsFound.length > 0) {
      if (confirm("Cambiar de cliente eliminará las partes agregadas o las encontradas. ¿Deseas continuar?")) {
        setUnitPartsAdded([]);
        setUnitPartsFound([]);
  ...
  const handleAddPart = (part: UnitPart) => {
    if (unitPartsAdded.some((p) => p.serial === part.serial && p.partId === part.part.id)) {
      addToast({
  ...
  const handleSearchPartByPartNumber = async () => {
    const response = await getUnitPartByPartNumber(partNumber, selectedClient as number, outputType);
    setUnitPartsFound(response.unitParts);
  }

  const handleSearchPartBySerial = async () => {
    const response = await getUnitPartBySerial(serial, selectedClient as number);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const outputData: NewOutput = {
      ...
      returnDate: outputType === "loan" && new Date(data.returnDate as string).toISOString() || undefined,
  ...
  return (
    <Form onSubmit={handleSubmit} className="flex flex-col gap-2 w-full max-w-sm md:max-w-md mx-auto h-full overflow-hidden">
      <Button
      ...
      <div className="flex flex-col gap-1 w-full">
        <p className={`text-sm after:ml-0.5 mb-1 after:text-red-500 after:content-['*'] ${errors.outputType !== "" && "text-danger-500"}`}>Tipo de salida</p>
        <div className="flex gap-2 md:gap-4 items-center justify-between">
          {
            outputTypes.map((type) => (
              <div key={type.value} onClick={() => {
                setErrors(
                  { ...errors, outputType: "" }
                );
                setOutputType(type.value);
              }} className={`flex items-center justify-center gap-1 md:gap-2 p-2 border-2 rounded-xl w-full cursor-pointer transition-all active:scale-95 ${styleOutputSelected(type.value)} ${errors.outputType !== "" && "border-[#f31260]"}`}>
                {type.icon}
      ...
      <Input
        value={outputType === "sale" || outputType === "loan" ? partNumber : outputType === "warranty" ? serial : ""}
        onChange={(e) => {
          if (outputType === "sale" || outputType === "loan") {
        ...
        placeholder={outputType === "sale" || outputType === "loan" ? "Ingresa el número de parte" : "Ingresa el serial de la parte"}
        startContent={<SearchIcon />}
        classNames={{
          inputWrapper: "pr-1",
        }}
          ...
          <div className="flex flex-col gap-2 h-fit overflow-y-auto">
            <p className="text-sm">Partes encontradas:</p>
            {
              unitPartsFound.map((unitPart) => (
                <div key={unitPart.id} className="flex border-1 p-2 gap-4 rounded-md">
                  <img
                  ...
                  <div className="flex flex-col gap-2 w-full">
                    ...
                    <p className="text-xs text-zinc-500">Serial: {unitPart.serial}</p>
                    <Button
                      size="sm"
                      variant="bordered"
                      color="primary"
      ...
      {
        Array.isArray(unitPartWarranty) && unitPartWarranty.length > 0 &&
        (
          unitPartWarranty.map((unitPartWarranty) =>
            <div className="flex border-1 p-2 gap-4 rounded-md w-full">
              ...
              <div className="flex flex-col gap-2 w-full">
                ...
                <p className="text-xs text-zinc-500">Serial: {unitPartWarranty.serial}</p>
                <Button
                  size="sm"
                  variant="bordered"
                  color="primary"
                  className="w-fit"
                  onPress={() => {
                    if (unitPartWarranty.part && unitPartWarranty.part.id !== undefined) {
                      setUnitPartsAdded([
                        ...unitPartsAdded,
                        {
                ...
                <div key={unitPart.id} className="flex border-1 p-2 gap-4 rounded-md">
                  ...
                  <div className="flex flex-col gap-2 w-full">
                    ...
                    <Button
                      ...
                      className="w-fit"
                      onPress={() => {
                        setUnitPartsAdded(unitPartsAdded.filter((p) => p.serial !== unitPart.serial));
                      }}
                    >
                      Eliminar
                    </Button>
