    ...
    <main className="flex flex-col gap-4 h-full overflow-hidden">
      ...
      <div className="flex flex-col md:grid md:grid-cols-2 gap-2 h-full overflow-hidden">
        <div className="flex flex-col gap-4 h-fit">
          {partSelected ? <p className="text-sm text-zinc-700">Parte seleccionada para el ingreso: <span className="font-semibold text-zinc-950">
            {partSelected.part.name}
          </span></p> : <p className="text-sm text-zinc-700">Seleccione una parte para continuar con el ingreso</p>}
          <Form onSubmit={handleSubmit} className="flex flex-col gap-4 h-full overflow-y-auto">
            <Input label="Número de parte" labelPlacement="outside" name="partNumber" placeholder="Número de parte de la selección" value={partSelected?.part.partNumber} readOnly isRequired variant="bordered" />
            <Input label="Serial" isRequired labelPlacement="outside" variant="bordered" name="serial" placeholder="Ingrese el serial de la parte recibida" />
            <div className="flex flex-col gap-2 w-full">
              ...
              <div className="flex flex-col gap-1">
                <span className="text-xs hidden sm:block md:mx-auto">Vista previa</span>
                <img src={imagePreview || "/default.png"} alt="Vista previa" className="hidden sm:block w-full md:w-1/2 md:mx-auto aspect-square bg-zinc-100 object-contain p-2 rounded-md" />
              </div>
            </div>
            <Button isLoading={isLoading} isDisabled={!partSelected} color="primary" className="w-full" type="submit">
        ...
        <div className="flex flex-col gap-2 h-full overflow-hidden">
          <p>Lista de partes</p>
          <p className="text-sm text-zinc-700">Selecciona una parte para continuar con el ingreso</p>
          <div className="flex flex-col gap-2 h-full overflow-y-auto">
            {
              quotationParts.length > 0 && quotationParts.map((quotationPart) => (
                quotationPart.quantity <= (quotationPart.receivedQuantity || 0) ?
                  null
                  :
                  <div onClick={() => {
                    setPartSelected(quotationPart);
                  }} className={`flex border-1 p-2 gap-4 rounded-md cursor-pointer active:scale-95 group transition-all ${styleSelect(quotationPart.id as number)}`} key={quotationPart.id}>
                    ...
                    <div className="flex flex-col gap-2 w-full">
                      ...
                      <p className="text-xs">
                        <span className="text-zinc-700">Productor:</span> {quotationPart.part.producer}
                      </p>
                      <p className="text-xs">
                        <span className="text-zinc-700">Cantidad:</span> {quotationPart.quantity - (quotationPart.receivedQuantity || 0)}
                      </p>
                      <p className="text-xs">
                        <span className="text-zinc-700">Cotización:</span> {quotationPart.clientQuotation.code}
                      </p>
                    </div>
                  </div>
              ))
